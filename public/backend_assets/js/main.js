...
function loadSearchData() {
  const menuType = $('#layout-menu').hasClass('menu-horizontal') ? 'horizontal' : 'vertical';
  ...
  // Use Laravel route instead of JSON file
  const searchUrl = baseUrl + '/search-menu-data?type=' + menuType;
  ...
  fetch(searchUrl, {
    ...
    .catch(error => {
      console.error('Error loading search data from controller:', error);
      // Fallback: try to load from JSON if controller fails
      const searchJson = menuType === 'horizontal' ? 'search-horizontal.json' : 'search-vertical.json';
      const fallbackUrl = baseUrl + '/backend_assets/json/' + searchJson;
      
      fetch(fallbackUrl)
        .then(response => {
          if (!response.ok) throw new Error('Failed to fetch fallback JSON: ' + response.status);
          return response.json();
        })
        .then(json => {
          data = json;
