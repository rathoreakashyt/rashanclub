$(async function () {
  ...
  function updateTotalItemCount() {
      const count = $('#stockInItems tr').length;
  ...
  // Function to add a new row to the stock in items table
  function addNewRow(itemData = null) {
      const rowCount = $('#stockInItems tr').length;
      const newRow = $('<tr></tr>');
  ...
  function updateRowNumbers() {
      $('#stockInItems tr').each(function(index) {
  ...
  $('#quickItemSelect').on('change', function() {
      ...
      if (itemExists(itemId)) {
          showErrorNotification('This item already exists in the stock in list.');
  ...
  $('#stockInForm').on('submit', function(e) {
      ...
      if ($('#stockInItems tr').length === 0) {
          errors.push('Please add at least one item to the stock in');
          isValid = false;
      } else {
