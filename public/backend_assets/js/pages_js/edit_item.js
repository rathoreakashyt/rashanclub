$(async function () {
    ...
    let isEditMode = typeof window.isEditMode !== 'undefined' && window.isEditMode === true;
    let itemData = typeof window.itemData !== 'undefined' ? window.itemData : null;
    let existingOpeningStock = typeof window.openingStockData !== 'undefined' ? window.openingStockData : {};
    let existingVariations = typeof window.variationsData !== 'undefined' ? window.variationsData : [];
    let existingComboItems = typeof window.comboItemsData !== 'undefined' ? window.comboItemsData : [];
    
    // Declare openingStockData early to avoid reference errors
    let openingStockData = {};
    
    // Initialize opening stock data for edit mode
    if (isEditMode && existingOpeningStock && Object.keys(existingOpeningStock).length > 0) {
        // Convert existing opening stock data to the format expected by the modal
        // The data comes from backend as {outlet_id: {quantity: X, items: [...]}}
        Object.keys(existingOpeningStock).forEach(function(outletId) {
            let outletData = existingOpeningStock[outletId];
            if (outletData) {
                ...
                // Handle items array
                if (outletData.items && Array.isArray(outletData.items)) {
                    outletData.items.forEach(function(item) {
                        // For Medicine products, item might have {value, quantity, expiry_date} or {quantity, expiry_date}
                        // For IMEI/Serial, item might be a string or {value: string}
                        if (typeof item === 'string') {
                            // IMEI/Serial format: direct string value (even if empty, preserve it for now)
                            if (item.trim() !== '') {
                                normalizedData.items.push(item);
                            }
                        } else if (item && typeof item === 'object') {
                            if (item.value !== undefined && item.expiry_date === undefined) {
                                // IMEI/Serial format: {value: string}
                                let valueStr = item.value ? item.value.toString() : '';
                                if (valueStr.trim() !== '') {
                                    normalizedData.items.push(valueStr);
                                }
                            } else if (item.quantity !== undefined && item.expiry_date !== undefined) {
                                // Medicine format: {quantity: X, expiry_date: Y}
                                normalizedData.items.push({
                                    quantity: item.quantity,
                                    expiry_date: item.expiry_date
                                });
                            } else if (item.expiry_date !== undefined) {
                                // Medicine format where value is expiry_date
                                normalizedData.items.push({
                                    quantity: item.quantity || 1,
                                    expiry_date: item.expiry_date || (item.value ? item.value.toString() : '')
                                });
                            } else {
                                // Try to extract value from object
                                let valueToAdd = null;
                                if (item.value !== undefined) {
                                    valueToAdd = item.value.toString();
                                } else if (item.toString) {
                                    valueToAdd = item.toString();
                                }
                                
                                if (valueToAdd && valueToAdd.trim() !== '') {
                                    normalizedData.items.push(valueToAdd);
                                }
                            }
                            ...
                            if (strValue && strValue.trim() !== '') {
                                normalizedData.items.push(strValue);
                            }
                        }
                    });
                }
                
                // Only add to openingStockData if there's actual data (quantity > 0 or items exist)
                if (normalizedData.quantity > 0 || normalizedData.items.length > 0) {
                    openingStockData[outletId] = normalizedData;
                }
            }
        });
    }
    /** #################### -- Edit Mode Initialization End --  #################### **/
    ...
    function updateSupplierSelect(suppliers, selectedId = null) {
        ...
        // Get current selected value from HTML (for edit mode)
        let currentSelected = selectedId || select.val() || select.find('option:selected').val();
        select.empty();
        ...
        suppliers.forEach(function(supplier) {
            ...
            if (currentSelected && supplier.id == currentSelected) {
    ...
    function updateBrandSelect(brands, selectedId = null) {
        ...
        brands.forEach(function(brand) {
            ...
            if (currentSelected && brand.id == currentSelected) {
                option.prop('selected', true);
            }
            select.append(option);
        });
    ...
    function updateCategorySelect(categories, selectedId = null) {
        ...
        categories.forEach(function(category) {
            ...
            if (currentSelected && category.id == currentSelected) {
                option.prop('selected', true);
            }
            select.append(option);
        });
    ...
    function updateUnitSelect(selector, units, selectedId = null) {
        ...
        units.forEach(function(unit) {
            ...
            if (currentSelected && unit.id == currentSelected) {
                option.prop('selected', true);
            }
            select.append(option);
        });
    ...
    function updateRackSelect(racks, selectedId = null) {
        ...
        racks.forEach(function(rack) {
            ...
            if (currentSelected && rack.id == currentSelected) {
                option.prop('selected', true);
            }
            select.append(option);
        });
    ...
    /** #################### -- Variation Product Variables --  #################### **/
    // Declare variation-related variables early to avoid reference errors
    let variationsData = [];
    let generatedVariations = [];
    let variationAttributesData = []; // Store variation attributes (rows) for variation_details
    /** #################### -- Variation Product Variables End --  #################### **/
    ...
    $(document).on('change', '#unit_type', function (e) {
        ...
        if($(this).val() == 1){
            ...
            $('.sale_unit_label').text(language_key['Unit']);
            ...
            $('#sale_unit_id').parent().find('.select2-selection__placeholder').text(language_key['Unit']);
        }else{
    ...
    if (isEditMode && itemData) {
        // Initialize with existing item type
        fieldShowHideByType(itemData.type);
        
        // Initialize unit type display (skip for Service_Product as it doesn't have units)
        if (itemData.unit_type && itemData.type !== 'Service_Product') {
            $('#unit_type').val(itemData.unit_type).trigger('change');
        }
        
        // Initialize opening stock summary if data exists
        if (openingStockData && Object.keys(openingStockData).length > 0) {
            setTimeout(function() {
                updateOpeningStockSummary(itemData.type);
            }, 1000);
        }
        
        // Load existing variations if Variation_Product
        if (itemData.type === 'Variation_Product') {
            // Show variation section
            $('.variation_section').show();
            
            if (existingVariations.length > 0) {
                setTimeout(function() {
                    loadExistingVariations(existingVariations);
                }, 1500);
            } else {
                // If no variations exist, show the variation generator
                $('.variation_generator').show();
                $('.generated_variations_section').hide();
            }
        }
        
        // Load existing combo items if Combo_Product
        if (itemData.type === 'Combo_Product' && existingComboItems.length > 0) {
            setTimeout(function() {
                loadExistingComboItems(existingComboItems);
            }, 1500);
        }
    } else {
    ...
    // Ensure variation section visibility is correct on initial load
    if ($('#type').val() !== 'Variation_Product') {
        $('.variation_section').hide();
        $('.generated_variations_section').hide();
    } else {
        // For Variation_Product, ensure sections are visible
        $('.variation_section').show();
    }
    ...
    // Variables are declared earlier to avoid reference errors
    // let variationsData = []; // Moved to top
    // let generatedVariations = []; // Moved to top
    // let variationAttributesData = []; // Moved to top
                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="file" class="form-control variation-image-input" 
                                    ...
                                    accept="image/*">
                                ${variation.encrypted_id ? `<input type="hidden" name="variations[${index}][id]" value="${variation.encrypted_id}">` : ''}
                                <div class="mt-2 variation-image-preview-${index}">
                                    ${variation.photo ? 
                                        `<img src="${base_url}/uploads/items/${variation.photo}" 
                                            alt="Variation Image" 
                                        ...
                                        <button type="button" class="btn btn-sm btn-danger mt-2 remove-variation-image" 
                                            data-variation-index="${index}" 
                                            style="${ variation.photo == '' ? 'display: none;' : ''}">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="opening-stock-display-${index} mt-2">
                                    ${variation.opening_stock && Object.keys(variation.opening_stock).length > 0 ? 
                                        (function() {
                                            let stockSummary = [];
                                            if (typeof window.outletsData !== 'undefined' && window.outletsData.length > 0) {
                                                window.outletsData.forEach(function(outlet) {
                                                    let quantity = variation.opening_stock[outlet.id] || 0;
                                                    if (quantity > 0) {
                                                        stockSummary.push(outlet.outlet_name + ': ' + quantity);
                                                    }
                                                });
                                            }
                                            return stockSummary.length > 0 ? 
                                                '<small class="text-muted">' + stockSummary.join(', ') + '</small>' : 
                                                '<small class="text-muted">No stock set</small>';
                                        })() : 
                                        '<small class="text-muted">No stock set</small>'
                                    }
                                </div>
    ...
    $(document).on('click', '.remove-variation-image', function() {
        ...
        $(`.variation-image-preview-${variationIndex} img`).attr('src', base_url + '/uploads/dummy_images/default-picture.png');
        $(this).hide();
    });
    ...
    $(document).on('click', '.set-opening-stock', function() {
        ...
        if (displayDiv.find('.opening-stock-edit').length > 0) {
            displayDiv.empty();
            $(this).html(`<i class="ti tabler-settings"></i> ${language_key['Set Opening Stock']}`);
        } else {
            displayDiv.html(`
    ...
    // Function to load existing variations for edit mode
    function loadExistingVariations(variations) {
        if (!variations || variations.length === 0) return;
        
        // Clear existing variation rows
        $('#variationAttributesBody').empty();
        
        // Group variations by variation_details to reconstruct the variation attributes
        // This is complex - we need to extract unique variation attributes from child items
        // For now, we'll show the generated variations directly
        generatedVariations = [];
        
        variations.forEach(function(variation, index) {
            let variationDetails = variation.variation_details;
            if (typeof variationDetails === 'string') {
                variationDetails = JSON.parse(variationDetails);
            }
            
            let variationName = variation.variation_name || '';
            let variationCombination = [];
            let variationAttributes = [];
            
            if (variationDetails) {
                if (variationDetails.variation_combination) {
                    variationCombination = variationDetails.variation_combination;
                }
                if (variationDetails.variation_attributes) {
                    variationAttributes = variationDetails.variation_attributes;
                }
            }
        ...
        if (generatedVariations.length > 0) {
            ...
            // If no variations loaded, show the generator
            $('.variation_generator').slideDown();
            $('.generated_variations_section').slideUp();
        }
    }
    
    // Function to load existing combo items for edit mode
    function loadExistingComboItems(comboItemsData) {
        if (!comboItemsData || comboItemsData.length === 0) return;
        
        comboItems = [];
        comboItemCounter = 0;
        $('#comboItemsBody').empty();
        
        comboItemsData.forEach(function(comboItemData) {
            comboItemCounter++;
            let comboItem = {
                sn: comboItemCounter,
                item_id: comboItemData.item_id,
                item_name: comboItemData.item_name,
                quantity: parseFloat(comboItemData.quantity) || 1,
                amount: parseFloat(comboItemData.amount) || 0,
                total: parseFloat(comboItemData.total) || 0,
                show_in_invoice: comboItemData.show_in_invoice !== undefined ? comboItemData.show_in_invoice : true
            };
    ...
    function validateRequiredFields() {
        ...
        if (!itemName || itemName.trim() === '') {
            ...
            if (nameWrapper.find('.invalid-feedback').length === 0) {
                nameWrapper.append('<div class="invalid-feedback">Item Name is required</div>');
            }
            hasError = true;
        } else {
        ...
        if (!categoryId || categoryId === '' || categoryId === null) {
            ...
            if (categoryWrapper.find('.invalid-feedback').length === 0) {
                categoryWrapper.append('<div class="invalid-feedback">Category is required</div>');
            }
            hasError = true;
        } else {
        ...
        if (itemType !== 'Service_Product' && itemType !== 'Combo_Product') {
            ...
            if (!unitType || unitType === '' || unitType === null) {
                ...
                // Validate unit fields based on unit type
                if (unitType == 1) {
                    ...
                    if (!saleUnitId || saleUnitId === '' || saleUnitId === null) {
                        ...
                        if (saleUnitWrapper.find('.invalid-feedback').length === 0) {
                            saleUnitWrapper.append('<div class="invalid-feedback">Unit is required</div>');
                        }
                        hasError = true;
                    } else {
                    ...
                    let purchaseUnitId = $('#purchase_unit_id').val();
                    let saleUnitId = $('#sale_unit_id').val();
                    let conversionRate = $('#conversion_rate').val();
                    
                    if (!purchaseUnitId || purchaseUnitId === '' || purchaseUnitId === null) {
                        ...
                        if (purchaseUnitWrapper.find('.invalid-feedback').length === 0) {
                            purchaseUnitWrapper.append('<div class="invalid-feedback">Purchase Unit is required</div>');
                        }
                        hasError = true;
                    } else {
                    ...
                    if (!saleUnitId || saleUnitId === '' || saleUnitId === null) {
                        ...
                        let saleUnitWrapper = $('#sale_unit_id').closest('.validate_wrapper');
                        if (saleUnitWrapper.find('.invalid-feedback').length === 0) {
                            saleUnitWrapper.append('<div class="invalid-feedback">' + language_key['Sale Unit is required'] + '</div>');
                        }
                        hasError = true;
                    } else {
                        $('#sale_unit_id').removeClass('is-invalid');
                    ...
                    if (!conversionRate || conversionRate.trim() === '' || isNaN(conversionRate) || parseFloat(conversionRate) <= 0) {
                        ...
                        if (conversionWrapper.find('.invalid-feedback').length === 0) {
                            conversionWrapper.append('<div class="invalid-feedback">Conversion Rate is required and must be greater than 0</div>');
                        }
                        hasError = true;
                    } else {
        ...
        if (itemType === 'Variation_Product') {
            if (!generatedVariations || generatedVariations.length === 0) {
                ...
                $('#generatedVariationsBody .col-12').each(function(index) {
                    ...
                    let variationMrpPrice = $(this).find('.variation-mrp-price').val();
                    if (variationMrpPrice && variationMrpPrice.trim() !== '' && (isNaN(variationMrpPrice) || parseFloat(variationMrpPrice) < 0)) {
                        $(this).find('.variation-mrp-price').addClass('is-invalid');
                        $(this).find('.variation-mrp-price').closest('.mb-3').append('<div class="invalid-feedback">MRP Price must be a valid positive number</div>');
                        hasError = true;
                    } else {
                        $(this).find('.variation-mrp-price').removeClass('is-invalid');
                        $(this).find('.variation-mrp-price').closest('.mb-3').find('.invalid-feedback').remove();
                    }
    ...
    $(document).on('submit', '#itemForm', function (e) {
        ...
        // Get item ID from form action URL
        let formAction = $('#itemForm').attr('action');
        let itemId = formAction.split('/').pop();
        const url_update = route('item.update', {item: itemId}, false, Ziggy);
        ...
        $.ajax({
            ...
            success: function (response, textStatus, xhr) {
                ...
                if (response && typeof response === 'object') {
                    if (response.status === 'success') {
                        window.location.href = base_url + '/item?status=success&message=Item updated successfully';
                    } else {
                        showErrorNotification(response.message || 'Something went wrong');
                    }
                } else {
                    // Response might be HTML (redirect), just redirect
                    window.location.href = base_url + '/item?status=success&message=Item updated successfully';
                }
            },
            error: function (xhr, textStatus, errorThrown) {
                ...
                if (xhr.status === 422) {
                    ...
                    // Server returned a redirect or success (sometimes redirects show as 200)
                    // Check if response is HTML (redirect) or JSON parse error
                    try {
                        let responseData = JSON.parse(xhr.responseText);
                        if (responseData.status === 'success') {
                            window.location.href = base_url + '/item?status=success&message=Item updated successfully';
                        } else {
                            showErrorNotification(responseData.message || 'Something went wrong');
                        }
                    } catch (e) {
                        // Response is HTML (redirect), just redirect
                        window.location.href = base_url + '/item?status=success&message=Item updated successfully';
                    }
                } else if (xhr.status === 0 || textStatus === 'parsererror') {
                    // CORS error or JSON parse error (might be redirect)
                    // Try to redirect anyway as the update might have succeeded
                    window.location.href = base_url + '/item?status=success&message=Item updated successfully';
                } else {
                    // Other errors
                    ...
                    if (xhr.responseJSON && xhr.responseJSON.message) {
                        ...
                        try {
                            ...
                            let errorElement = doc.querySelector('.alert-danger, .error, [role="alert"]');
                            if (errorElement) {
                                errorMessage = errorElement.textContent.trim();
                            }
                        } catch (e) {
                            // If parsing fails, check if it's a redirect response
                            if (xhr.responseText.includes('redirect') || xhr.responseText.includes('location')) {
                                window.location.href = base_url + '/item?status=success&message=Item updated successfully';
                                return;
                            }
                        }
    ...
    $(document).on('click', '.remove-image', function() {
        $('#photo').val('');
        $('#photo_preview').attr('src', base_url + '/uploads/dummy_images/default-picture.png');
        $(this).hide();
    });
    ...
    // openingStockData is declared earlier in Edit Mode Initialization section
            ...
            if (itemType === 'General_Product' || itemType === 'Installment_Product' || itemType === 'Combo_Product' || (itemType === 'Medicine_Product' && expiryDateMaintain === 'No')) {
                ...
                if (outletData && outletData.items && outletData.items.length > 0) {
                    outletData.items.forEach(function(item) {
                        ...
                        let itemValue = typeof item === 'string' ? item : (item.value || item);
                        if (itemValue) {
                            // Skip duplicate check when loading existing data
                            addIMEISerialField(outletId, itemValue, fieldLabel, true);
                        }
                    });
                }
            } else if (itemType === 'Medicine_Product' && expiryDateMaintain === 'Yes') {
                // Medicine product - similar to IMEI/Serial but with Quantity + MM/YY + Expiry Date (design matches add page)
                outletCard.html(`
                ...
                if (outletData && outletData.items && outletData.items.length > 0) {
                    outletData.items.forEach(function(item) {
                        ...
                        let expiryDate = item.expiry_date || item.value || null;
                        
                        // If item is an object with value as expiry_date and quantity separately
                        if (item.value && !item.expiry_date && item.quantity) {
    ...
    $(document).on('click', '#setOpeningStockBtn', function () {
        ...
        // Pass openingStockData which contains the existing stock data
        initializeOpeningStockModal(itemType, openingStockData);
        $('#modal_outlet').modal('show');
    });
    
    // Reset opening stock when product type changes
    // Note: In edit mode, type is disabled, so this handler may not be needed, but keeping for consistency
            ...
            if (openingStockData && Object.keys(openingStockData).length > 0) {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        // User cancelled, keep previous value
                        $(this).val(previousValue);
    ...
    function addIMEISerialField(outletId, value, fieldLabel, skipDuplicateCheck = false) {
        // Check for duplicates (skip check when loading existing data)
        if (!skipDuplicateCheck && allIMEISerialNumbers.includes(value)) {
            showErrorNotification(`${fieldLabel} number "${value}" already exists. Please enter a unique ${fieldLabel}.`);
            return;
        }
        
        // Only add to tracking array if not already present
    ...
    function generateOpeningStockSummary(itemType) {
        ...
        if (itemType === 'General_Product' || itemType === 'Installment_Product' || itemType === 'Combo_Product' || (itemType === 'Medicine_Product' && expiryDateMaintain === 'No')) {
            ...
            Object.keys(openingStockData).forEach(function(outletId) {
                let outletData = openingStockData[outletId];
                // Check if items exist and filter out empty/null values
                if (outletData && outletData.items && Array.isArray(outletData.items)) {
                    // Filter out empty, null, or undefined items
                    let validItems = outletData.items.filter(function(item) {
                        if (typeof item === 'string') {
                            return item.trim() !== '';
                    ...
                    if (validItems.length > 0) {
                        let outletName = getOutletName(outletId);
                        summaryHtml += `<li><strong>${outletName}:</strong> ${validItems.length} ${fieldLabel}(s)</li>`;
                        hasAnyStock = true;
                    }
                }
            });
