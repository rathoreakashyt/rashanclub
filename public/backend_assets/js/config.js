...
// Get assets path and ensure it's absolute
let assetsPathAttr = document.documentElement.getAttribute('data-assets-path');
if (assetsPathAttr) {
  // If it's a relative path (starts with /../ or ../), convert to absolute
  if (assetsPathAttr.startsWith('/../') || assetsPathAttr.startsWith('../')) {
    // Remove leading /../ or ../
    const cleanPath = assetsPathAttr.replace(/^\/?\.\.\//, '');
    // Get base URL from current location
    const baseUrl = window.location.origin;
    // Construct absolute path
    window.assetsPath = baseUrl + '/' + cleanPath;
  } else if (!assetsPathAttr.startsWith('http://') && !assetsPathAttr.startsWith('https://') && !assetsPathAttr.startsWith('/')) {
    // Relative path without /../, make it absolute
    const baseUrl = window.location.origin;
    window.assetsPath = baseUrl + '/' + assetsPathAttr;
  } else {
    // Already absolute or starts with /
    window.assetsPath = assetsPathAttr;
  }
  // Ensure assetsPath ends with a slash
