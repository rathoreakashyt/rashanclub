$(async function () {
    ...
    let stockReportRoute;
    let reportSearchItemsUrl;
    try {
        stockReportRoute = route('report.stock-report', {}, false, Ziggy);
        ...
        stockReportRoute = '/reports/stock-report';
    ...
    function getExportHeader() {
        var header = '<div style="margin-bottom: 20px;"><h3 style="text-align: center; margin-bottom: 15px;">' + (language_key.StockReport || 'Stock Report') + '</h3>';
    ...
    function updateFooterTotals(summary) {
        if (summary) {
            $('#stock_value_span').text(summary.stock_value || '0.00');
            $('#stock_count_span').text(summary.stock_count || '0');
        }
    }
    ...
    function loadReportData() {
        ...
        allRows = [];
        totalCount = 0;
        $('#stockReportLoading').removeClass('d-none');
        $('#stockReportVirtualWrap').addClass('d-none');

        fetchChunk(0).then(function (response) {
            if (loadAborted || !response.success || !response.data) {
                $('#stockReportLoading').addClass('d-none');
                return;
            }
            ...
            $('#stockReportLoading').addClass('d-none');
            $('#filteredInformation').hide();
            updateFooterTotals({ stock_value: '0.00', stock_count: '0' });
        });
        ...
        function finishLoad() {
            ...
            buildVirtualDom();
            $('#stockReportVirtualWrap').removeClass('d-none');
            renderVisibleRows();
        }
    }

    function buildVirtualDom() {
        var wrap = $('#stockReportTableVirtual');
        wrap.empty();
    ...
    $('#stockReportFilterBtn').on('click', function () {
        $('#filterSection').slideToggle();
        setTimeout(function () { if ($('#filterSection').is(':visible')) initSelect2(); }, 200);
    });
    $(document).on('click', '#stockReportExportBtn, [data-export]', function (e) {
        ...
        if (exportType === 'print') {
            ...
            win.document.write('<html><head><title>' + (language_key.StockReport || 'Stock Report') + '</title><link rel="stylesheet" href="' + base_url + '/public/backend_assets/css/bootstrap.min.css"/></head><body>');
            ...
            var csv = '\uFEFF';
            csv += (language_key.StockReport || 'Stock Report') + '\n';
            ...
            pdfMake.createPdf({
                ...
                content: [
                    { text: (language_key.StockReport || 'Stock Report'), style: 'header' },
    ...
    initSelect2();
    checkAndShowFilters();
    // Load full report on first load when no filters are applied
    var hasFilter = $('#category_id_f').val() || $('#brand_id_f').val() || $('#item_id_f').val() || $('#generic_name_f').val() || $('#supplier_id_f').val();
