  ...
  if (dt_basic_table) {
    new DataTable(dt_basic_table, {
      ...
      ajax: {
        url: base_url + '/income',
        type: 'GET',
        error: function(xhr, error, thrown) { console.error('list_income AJAX error:', error, thrown); }
      },
      columns: [
        { data: 'id' },
        { data: 'reference_no' },
        ...
        { 
          ...
          render: function(data) {
            ...
            const edit_url = route('income.edit', { income: data.encrypted_id }, false, Ziggy);
            const destroy_url = route('income.destroy', { income: data.encrypted_id }, false, Ziggy);
      ...
      buttons: [
        ...
        {
          text: '<i class="ti tabler-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">'+language_key['Add Income']+'</span>',
          className: 'btn create-new btn-primary',
          action: function (e, dt, node, config) {
            window.location.href = base_url + '/income/create';
          }
        }
      ],
