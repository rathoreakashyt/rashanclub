  ...
  if (dt_basic_table) {
    new DataTable(dt_basic_table, {
      ...
      ajax: {
        ...
        type: 'GET',
        error: function(xhr, error, thrown) { console.error('list_quotation AJAX error:', error, thrown); }
      },
      columns: [
        { data: 'id' },
        { data: 'reference_no' },
        ...
        { 
          ...
          render: function(data, type, row) {
            return data + (row.customer_phone ? ' (' + row.customer_phone + ')' : '');
          }
        },
        { data: 'grand_total' },
        { 
          ...
          render: function(data) {
            ...
            const edit_url = route('quotation.edit', { quotation: data.encrypted_id }, false, Ziggy);
            const destroy_url = route('quotation.destroy', { quotation: data.encrypted_id }, false, Ziggy);
            const show_url = route('quotation.show', { quotation: data.encrypted_id }, false, Ziggy);
            const print_url = route('quotation.print-invoice', { quotation: data.encrypted_id }, false, Ziggy);
            // download pdf url
            const download_pdf_url = route('quotation.generate-pdf', { quotation: data.encrypted_id }, false, Ziggy);
            return `<div class="d-inline-flex datatable-action">
              <a href="${base_url+download_pdf_url}" class="btn btn-sm btn-icon download-pdf">
                <i class="ti tabler-book-download"></i>
              </a>
              <a href="${base_url+print_url}" class="btn btn-sm btn-icon">
                <i class="ti tabler-printer"></i>
              </a>
              <a href="${base_url+show_url}" class="btn btn-sm btn-icon">
      ...
      buttons: [
        ...
        {
          text: '<i class="ti tabler-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">'+language_key['Add Quotation']+'</span>',
