$(async function () {
  ...
  if (dt_basic_table) {
    new DataTable(dt_basic_table, {
      ...
      serverSide: true,
      ajax: {
        url: base_url + '/supplier-payment',
        type: 'GET',
        error: function(xhr, error, thrown) { console.error('list_supplier_payment AJAX error:', error, thrown); }
      },
      columns: [
        { data: 'id' },
        { data: 'reference_no' },
        ...
        { 
          ...
          render: function(data) {
            ...
            const edit_url = route('supplier-payment.edit', { supplier_payment: data.encrypted_id }, false, Ziggy);
            const destroy_url = route('supplier-payment.destroy', { supplier_payment: data.encrypted_id }, false, Ziggy);
      ...
      buttons: [
        ...
        {
          text: '<i class="ti tabler-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">'+language_key['Add Supplier Payment']+'</span>',
          className: 'btn create-new btn-primary',
          action: function (e, dt, node, config) {
            window.location.href = base_url + '/supplier-payment/create';
          }
        }
      ],
