$(async function () {
  ...
  if (dt_basic_table) {
    new DataTable(dt_basic_table, {
      ...
      serverSide: true,
      ajax: {
        url: base_url + '/delivery-partner',
        type: 'GET',
        error: function(xhr, error, thrown) { console.error('list_delivary_partner AJAX error:', error, thrown); }
      },
      columns: [
        { data: 'id' },
        { data: 'partner_name' },
        ...
        { 
          ...
          render: function(data) {
            ...
            const edit_url = route('delivery-partner.edit', { delivery_partner: data.encrypted_id }, false, Ziggy);
            const destroy_url = route('delivery-partner.destroy', { delivery_partner: data.encrypted_id }, false, Ziggy);
      ...
      buttons: [
        ...
        {
          text: '<i class="ti tabler-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">'+language_key['Add Delivery Partner']+'</span>',
          className: 'btn create-new btn-primary',
          action: function (e, dt, node, config) {
            window.location.href = base_url + '/delivery-partner/create';
          }
        }
      ],
