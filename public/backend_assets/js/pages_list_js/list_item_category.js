$(async function () {
  ...
  const route_path = route('item-category.index', {}, false, Ziggy);
  if (dt_basic_table) {
    new DataTable(dt_basic_table, {
      ...
      ajax: {
        ...
        type: 'GET',
        error: function(xhr, error, thrown) { console.error('list_item_category AJAX error:', error, thrown); }
      },
      columns: [
        { data: 'id' },
        { data: 'name' },
        ...
        { 
          ...
          render: function(data) {
            ...
            const edit_url = route('item-category.edit', { item_category: data.encrypted_id }, false, Ziggy);
            const destroy_url = route('item-category.destroy', { item_category: data.encrypted_id }, false, Ziggy);
      ...
      buttons: [
        ...
        {
          text: '<i class="ti tabler-sort-descending me-sm-1"></i> <span class="d-none d-sm-inline-block">'+language_key['Sort Item Category']+'</span>',
          className: 'btn create-new btn-primary me-2',
          action: function (e, dt, node, config) {
            const sortRoute = route('item-category.sort-category', {}, false, Ziggy);
            window.location.href = base_url + sortRoute;
          }
        },
        {
          text: '<i class="ti tabler-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">'+language_key['Add Item Category']+'</span>',
          className: 'btn create-new btn-primary',
          action: function (e, dt, node, config) {
            window.location.href = base_url + '/item-category/create';
          }
        }
      ],
