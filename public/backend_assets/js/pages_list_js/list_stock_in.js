$(async function () {
  ...
  if (dt_basic_table) {
    new DataTable(dt_basic_table, {
      ...
      serverSide: true,
      ajax: {
        url: base_url + '/fixed-asset-stock-in',
        type: 'GET',
        error: function(xhr, error, thrown) { console.error('list_stock_in AJAX error:', error, thrown); }
      },
      columns: [
        { data: 'id' },
        { data: 'reference_no' },
        ...
        { 
          ...
          render: function(data) {
            ...
            const edit_url = route('fixed-asset-stock-in.edit', { fixed_asset_stock_in: data.encrypted_id }, false, Ziggy);
            const destroy_url = route('fixed-asset-stock-in.destroy', { fixed_asset_stock_in: data.encrypted_id }, false, Ziggy);
            const show_url = route('fixed-asset-stock-in.show', { fixed_asset_stock_in: data.encrypted_id }, false, Ziggy);
      ...
      buttons: [
        ...
        {
          text: '<i class="ti tabler-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">'+language_key['Add Stock In']+'</span>',
          className: 'btn create-new btn-primary',
          action: function (e, dt, node, config) {
            window.location.href = base_url + '/fixed-asset-stock-in/create';
          }
        }
      ],
