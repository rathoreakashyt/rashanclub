  ...
  if (dt_basic_table) {
    new DataTable(dt_basic_table, {
      ...
      ajax: {
        url: base_url + '/damage',
        type: 'GET',
        error: function(xhr, error, thrown) { console.error('list_damage AJAX error:', error, thrown); }
      },
      columns: [
        { data: 'id' },
        { data: 'reference_no' },
        ...
        { 
          ...
          render: function(data, type, row) {
            return data + (row.responsible_person_phone ? ' (' + row.responsible_person_phone + ')' : '');
          }
        },
        { data: 'total_loss' },
        { 
          ...
          render: function(data) {
            ...
            const edit_url = route('damage.edit', { damage: data.encrypted_id }, false, Ziggy);
            const destroy_url = route('damage.destroy', { damage: data.encrypted_id }, false, Ziggy);
            const show_url = route('damage.show', { damage: data.encrypted_id }, false, Ziggy);
      ...
      buttons: [
        ...
        {
          text: '<i class="ti tabler-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">'+language_key['Add Damage']+'</span>',
          className: 'btn create-new btn-primary',
          action: function (e, dt, node, config) {
            window.location.href = base_url + '/damage/create';
          }
        }
      ],
