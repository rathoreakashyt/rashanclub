$(async function () {
  ...
  if (dt_basic_table) {
    new DataTable(dt_basic_table, {
      ...
      serverSide: true,
      ajax: {
        url: base_url + '/servicing',
        type: 'GET',
        error: function(xhr, error, thrown) { console.error('list_servicing AJAX error:', error, thrown); }
      },
      columns: [
        { data: 'id' },
        { data: 'date' },
        ...
        { 
          ...
          render: function(data) {
            ...
            const edit_url = route('servicing.edit', { servicing: data.encrypted_id }, false, Ziggy);
            const destroy_url = route('servicing.destroy', { servicing: data.encrypted_id }, false, Ziggy);
      ...
      buttons: [
        ...
        {
          text: '<i class="ti tabler-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">'+language_key['Add Servicing']+'</span>',
          className: 'btn create-new btn-primary',
          action: function (e, dt, node, config) {
            window.location.href = base_url + '/servicing/create';
          }
        }
      ],
