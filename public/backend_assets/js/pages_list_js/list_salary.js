$(async function () {
  ...
  const route_path = route('salary.index', {}, false, Ziggy);
  ...
  if (dt_basic_table) {
    new DataTable(dt_basic_table, {
      ...
      ajax: {
        ...
        type: 'GET',
        error: function(xhr, error, thrown) { console.error('list_salary AJAX error:', error, thrown); }
      },
      columns: [
        ...
        { 
          ...
          render: function(data) {
            ...
            const edit_url = route('salary.edit', { salary: data.encrypted_id }, false, Ziggy);
            const destroy_url = route('salary.destroy', { salary: data.encrypted_id }, false, Ziggy);
            const show_url = route('salary.show', { salary: data.encrypted_id }, false, Ziggy);
      ...
      buttons: [
        ...
        {
          text: '<i class="ti tabler-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">'+language_key['Add Salary']+'</span>',
          className: 'btn create-new btn-primary',
          action: function (e, dt, node, config) {
            window.location.href = base_url + route('salary.create', {}, false, Ziggy);
          }
        }
      ],
