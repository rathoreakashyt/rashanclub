$(async function () {
  ...
  if (dt_basic_table) {
    new DataTable(dt_basic_table, {
      ...
      serverSide: true,
      ajax: {
        url: base_url + '/denomination',
        type: 'GET',
        error: function(xhr, error, thrown) { console.error('list_denomination AJAX error:', error, thrown); }
      },
      columns: [
        { data: 'id' },
        { data: 'amount' },
        ...
        { 
          ...
          render: function(data) {
            ...
            const edit_url = route('denomination.edit', { denomination: data.encrypted_id }, false, Ziggy);
            const destroy_url = route('denomination.destroy', { denomination: data.encrypted_id }, false, Ziggy);
      ...
      buttons: [
        ...
        {
          text: '<i class="ti tabler-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">'+language_key['Add Denomination']+'</span>',
          className: 'btn create-new btn-primary',
          action: function (e, dt, node, config) {
            window.location.href = base_url + '/denomination/create';
          }
        }
      ],
