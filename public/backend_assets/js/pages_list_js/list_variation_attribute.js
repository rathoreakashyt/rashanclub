$(async function () {
  ...
  const route_path = route('variation-attribute.index', {}, false, Ziggy);
  ...
  if (dt_basic_table) {
    new DataTable(dt_basic_table, {
      ...
      ajax: {
        ...
        type: 'GET',
        error: function(xhr, error, thrown) { console.error('list_variation_attribute AJAX error:', error, thrown); }
      },
      columns: [
        { data: 'id' },
        { data: 'variation_name' },
        ...
        { 
          ...
          render: function(data) {
            ...
            const edit_url = route('variation-attribute.edit', { variation_attribute: data.encrypted_id }, false, Ziggy);
            const destroy_url = route('variation-attribute.destroy', { variation_attribute: data.encrypted_id }, false, Ziggy);
      ...
      buttons: [
        ...
        {
          text: '<i class="ti tabler-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">'+language_key['Add Variation Attribute']+'</span>',
          className: 'btn create-new btn-primary',
          action: function (e, dt, node, config) {
            window.location.href = base_url + '/variation-attribute/create';
          }
        }
      ],
