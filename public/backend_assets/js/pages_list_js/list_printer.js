  ...
  if (dt_basic_table) {
    new DataTable(dt_basic_table, {
      ...
      ajax: {
        ...
        type: 'GET',
        error: function(xhr, error, thrown) { console.error('list_printer AJAX error:', error, thrown); }
      },
      columns: [
        { data: 'id' },
        { data: 'title' },
        ...
        { 
          ...
          render: function(data) {
            ...
            const edit_url = route('printer.edit', { printer: data.encrypted_id }, false, Ziggy);
            const destroy_url = route('printer.destroy', { printer: data.encrypted_id }, false, Ziggy);
      ...
      buttons: [
        ...
        {
          text: '<i class="ti tabler-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">'+language_key['Add Printer']+'</span>',
          className: 'btn create-new btn-primary',
          action: function (e, dt, node, config) {
            window.location.href = base_url + '/printer/create';
          }
        }
      ],
