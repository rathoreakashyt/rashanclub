$(async function () {
  ...
  if (dt_basic_table) {
    new DataTable(dt_basic_table, {
      ...
      serverSide: true,
      ajax: {
        url: base_url + '/fixed-asset-item',
        type: 'GET',
        error: function(xhr, error, thrown) { console.error('list_fixed_items AJAX error:', error, thrown); }
      },
      columns: [
        { data: 'id' },
        { data: 'name' },
        ...
        { 
          ...
          render: function(data) {
            ...
            const edit_url = route('fixed-asset-item.edit', { fixed_asset_item: data.encrypted_id }, false, Ziggy);
            const destroy_url = route('fixed-asset-item.destroy', { fixed_asset_item: data.encrypted_id }, false, Ziggy);
      ...
      buttons: [
        ...
        {
          text: '<i class="ti tabler-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">'+language_key['Add Fixed Asset Item']+'</span>',
          className: 'btn create-new btn-primary',
          action: function (e, dt, node, config) {
            window.location.href = base_url + '/fixed-asset-item/create';
          }
        }
      ],
