$(async function () {
  ...
  if (dt_basic_table) {
    new DataTable(dt_basic_table, {
      ...
      serverSide: true,
      ajax: {
        url: base_url + '/multiple-currency',
        type: 'GET',
        error: function(xhr, error, thrown) { console.error('list_multiple_currency AJAX error:', error, thrown); }
      },
      columns: [
        { data: 'id' },
        { data: 'currency' },
        ...
        { 
          ...
          render: function(data) {
            ...
            const edit_url = route('multiple-currency.edit', { multiple_currency: data.encrypted_id }, false, Ziggy);
            const destroy_url = route('multiple-currency.destroy', { multiple_currency: data.encrypted_id }, false, Ziggy);
      ...
      buttons: [
        ...
        {
          text: '<i class="ti tabler-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">'+language_key['Add Multiple Currency']+'</span>',
          className: 'btn create-new btn-primary',
          action: function (e, dt, node, config) {
            window.location.href = base_url + '/multiple-currency/create';
          }
        }
      ],
