$(async function () {
  ...
  if (dt_basic_table) {
    new DataTable(dt_basic_table, {
      ...
      serverSide: true,
      ajax: {
        url: base_url + '/installment-customer',
        type: 'GET',
        error: function(xhr, error, thrown) { console.error('list_installment_customer AJAX error:', error, thrown); }
      },
      columns: [
        { data: 'id' },
        { data: 'name' },
        ...
        { 
          ...
          render: function(data) {
            ...
            const edit_url = route('installment-customer.edit', { installment_customer: data.encrypted_id }, false, Ziggy);
            const destroy_url = route('installment-customer.destroy', { installment_customer: data.encrypted_id }, false, Ziggy);
      ...
      buttons: [
        ...
        {
          text: '<i class="ti tabler-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">'+language_key['Add Installment Customer']+'</span>',
          className: 'btn create-new btn-primary',
          action: function (e, dt, node, config) {
            window.location.href = base_url + '/installment-customer/create';
          }
        }
      ],
