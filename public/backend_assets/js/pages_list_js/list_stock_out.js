$(async function () {
  ...
  if (dt_basic_table) {
    new DataTable(dt_basic_table, {
      ...
      serverSide: true,
      ajax: {
        url: base_url + '/fixed-asset-stock-out',
        type: 'GET',
        error: function(xhr, error, thrown) { console.error('list_stock_out AJAX error:', error, thrown); }
      },
      columns: [
        { data: 'id' },
        { data: 'reference_no' },
        ...
        { 
          ...
          render: function(data) {
            ...
            const edit_url = route('fixed-asset-stock-out.edit', { fixed_asset_stock_out: data.encrypted_id }, false, Ziggy);
            const destroy_url = route('fixed-asset-stock-out.destroy', { fixed_asset_stock_out: data.encrypted_id }, false, Ziggy);
            const show_url = route('fixed-asset-stock-out.show', { fixed_asset_stock_out: data.encrypted_id }, false, Ziggy);
      ...
      buttons: [
        ...
        {
          text: '<i class="ti tabler-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">'+language_key['Add Stock Out']+'</span>',
          className: 'btn create-new btn-primary',
          action: function (e, dt, node, config) {
            window.location.href = base_url + '/fixed-asset-stock-out/create';
          }
        }
      ],
