  ...
  if (dt_basic_table) {
    new DataTable(dt_basic_table, {
      ...
      ajax: {
        ...
        type: 'GET',
        error: function(xhr, error, thrown) { console.error('list_promotion AJAX error:', error, thrown); }
      },
      columns: [
        { data: 'id' },
        { data: 'title' },
        ...
        { 
          ...
          render: function(data) {
            return data == 1 ? 
              '<span class="badge bg-label-success">' + language_key.Enable + '</span>' : 
              '<span class="badge bg-label-danger">' + language_key.Disable + '</span>';
          }
        },
        { 
          data: null,
          ...
          render: function(data) {
            ...
            const edit_url = route('promotion.edit', { promotion: data.encrypted_id }, false, Ziggy);
            const destroy_url = route('promotion.destroy', { promotion: data.encrypted_id }, false, Ziggy);
      ...
      buttons: [
        ...
        {
          text: '<i class="ti tabler-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">'+language_key['Add Promotion']+'</span>',
          className: 'btn create-new btn-primary',
          action: function (e, dt, node, config) {
            window.location.href = base_url + '/promotion/create';
          }
        }
      ],
