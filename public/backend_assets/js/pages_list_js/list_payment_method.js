  ...
  if (dt_basic_table) {
    new DataTable(dt_basic_table, {
      ...
      ajax: {
        ...
        type: 'GET',
        error: function(xhr, error, thrown) { console.error('list_payment_method AJAX error:', error, thrown); }
      },
      columns: [
        { data: 'id' },
        { data: 'name' },
        { data: 'account_type', render: function(data) {
        ...
        { 
          ...
          render: function(data) {
            ...
            const edit_url = route('payment-method.edit', { payment_method: data.encrypted_id }, false, Ziggy);
            const destroy_url = route('payment-method.destroy', { payment_method: data.encrypted_id }, false, Ziggy);
            
            let deleteButton = '';
            if(data.is_deletable == 'Yes'){
              deleteButton = `
      ...
      buttons: [
        ...
        {
          text: '<i class="ti tabler-arrows-sort me-sm-1"></i> <span class="d-none d-sm-inline-block">'+language_key['Sort Payment Method']+'</span>',
          className: 'btn btn-primary me-2',
          action: function (e, dt, node, config) {
            window.location.href = base_url + '/payment-method/sort-payment-method';
          }
        }, 
        {
          text: '<i class="ti tabler-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">'+language_key['Add Payment Method']+'</span>',
          className: 'btn create-new btn-primary',
          action: function (e, dt, node, config) {
            window.location.href = base_url + '/payment-method/create';
          }
        }
      ],
