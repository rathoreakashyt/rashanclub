$(async function () {
  ...
  const route_path = route('unit.index', {}, false, Ziggy);
  ...
  if (dt_basic_table) {
    new DataTable(dt_basic_table, {
      ...
      ajax: {
        ...
        type: 'GET',
        error: function(xhr, error, thrown) { console.error('list_unit AJAX error:', error, thrown); }
      },
      columns: [
        { data: 'id' },
        { data: 'unit_name' },
        ...
        { 
          ...
          render: function(data) {
            ...
            const edit_url = route('unit.edit', { unit: data.encrypted_id }, false, Ziggy);
            const destroy_url = route('unit.destroy', { unit: data.encrypted_id }, false, Ziggy);
      ...
      buttons: [
        {
          ...
          buttons: [
            ...
            {
              extend: 'csv',
              text: '<i class="ti tabler-file-text me-1" ></i>'+language_key.Csv,
              className: 'dropdown-item',
              exportOptions: { columns: [0, 1, 2] }
            },
            {
            ...
            {
              extend: 'copy',
              text: '<i class="ti tabler-copy me-1" ></i>'+language_key.Copy,
              className: 'dropdown-item',
              exportOptions: { columns: [0, 1, 2] }
            }
          ]
        ...
        {
          text: '<i class="ti tabler-plus me-sm-1"></i> <span class="d-none d-sm-inline-block">'+language_key['Add Unit']+'</span>',
          className: 'btn create-new btn-primary',
          action: function (e, dt, node, config) {
            window.location.href = base_url + '/unit/create';
          }
        }
      ],
