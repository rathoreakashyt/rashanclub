...
{
    public partial class SaleReportPage : UserControl
    {
        private readonly MainDashboard? _dashboard;
        private readonly DatabaseService _db = new();
        private DataTable? _currentTable;

        public SaleReportPage(MainDashboard? dashboard)
        {
            InitializeComponent();
            ReportPdfHelper.Attach(exportPopup, () => dataGrid.ItemsSource as DataView, "Sale Report");
        ...
        private void LoadData()
        {
            ...
            var sql = "SELECT ROW_NUMBER() OVER (ORDER BY s.sale_date DESC, s.Id DESC) AS sn,"
                + " s.sale_no, s.sale_date, IFNULL(c.name,'Walk-in') AS customer,"
                ...
                + " ROUND(IFNULL(s.vat,0),2) AS tax,"
                + " ROUND(IFNULL(s.delivery_charge,0),2) AS charge,"
                + " ROUND(IFNULL(s.total_discount_amount,0),2) AS discount,"
                + " ROUND(IFNULL(s.total_payable,0),2) AS total_payable,"
                + " ROUND(IFNULL(s.paid_amount,0),2) AS paid_amount,"
                + " ROUND(IFNULL(s.due_amount,0),2) AS due_amount"
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show("Error: " + ex.Message, "Sale Report", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        {
            lblTotalSubtotal.Text = $"₹ {ColSum(t, "subtotal"):N2}";
            lblTotalTax.Text      = $"₹ {ColSum(t, "tax"):N2}";
            lblTotalDiscount.Text = $"₹ {ColSum(t, "discount"):N2}";
            lblTotalPayable.Text  = $"₹ {ColSum(t, "total_payable"):N2}";
            lblTotalPaid.Text     = $"₹ {ColSum(t, "paid_amount"):N2}";
            lblTotalDue.Text      = $"₹ {ColSum(t, "due_amount"):N2}";
        ...
        {
            var sp = new System.Windows.Controls.StackPanel { Orientation = Orientation.Horizontal, Margin = new Thickness(0, 0, 0, 2) };
        ...
        private void BtnExportCsv_Click(object sender, RoutedEventArgs e)
        {
            ...
            var dlg = new SaveFileDialog { Filter = "CSV (*.csv)|*.csv", FileName = "SaleReport_" + DateTime.Today.ToString("yyyyMMdd") + ".csv" };
        ...
        private void BtnPrint_Click(object sender, RoutedEventArgs e)
        {
            ...
            doc.Blocks.Add(new Paragraph(new Run("Sale Report")) { FontSize = 16, FontWeight = FontWeights.Bold, Margin = new Thickness(0,0,0,6) });
            ...
            dlg.PrintDocument(pag, "Sale Report");
        }
    }
}
