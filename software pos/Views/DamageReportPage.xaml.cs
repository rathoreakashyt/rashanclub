...
{
    public partial class DamageReportPage : UserControl
    {
        private readonly MainDashboard _dashboard;
        private readonly DatabaseService _db = new();
        private DataTable _allData = new DataTable();

        public DamageReportPage(MainDashboard dashboard)
        {
            InitializeComponent();
            ReportPdfHelper.Attach(exportPopup, () => dataGrid.ItemsSource as DataView, "Damage Report");
        ...
        private void LoadReport(DateTime? dateFrom = null, DateTime? dateTo = null, string outlet = "", long employeeId = 0, long outletId = 0)
        {
            try
            {
                ...
                string empCond = employeeId > 0 ? $" AND d.employee_id={employeeId}" : "";
                ...
                dt.Columns.Add("total_loss", typeof(double));
                dt.Columns.Add("responsible_person", typeof(string));
                dt.Columns.Add("_lossRaw", typeof(double));
                ...
                using (var cmd = conn.CreateCommand())
                {
                    cmd.CommandText = $@"SELECT d.id, d.reference_no, d.created_at, d.total_loss, d.employee_id,
                        (SELECT COUNT(*) FROM damage_details dd WHERE dd.damage_id=d.id AND (dd.del_status IS NULL OR dd.del_status='Live')) AS total_items
                        FROM damages d
                        WHERE (d.del_status IS NULL OR d.del_status='Live')
                        {dateCond("d.date")}
                        {outletCond("d")}
                        {empCond}
                    ...
                    while (r.Read())
                    {
                        ...
                        double loss = r.IsDBNull(3) ? 0 : Convert.ToDouble(r.GetValue(3));
                        long empId = r.IsDBNull(4) ? 0 : Convert.ToInt64(r.GetValue(4));
                        int items = r.IsDBNull(5) ? 0 : Convert.ToInt32(r.GetValue(5));
                        ...
                        tLoss += loss;
                        dt.Rows.Add(sn, refNo, dtStr, items, loss, person, loss);
                    }
                }
                ...
                lblTotalLoss.Text = "₹ " + tLoss.ToString("N2");
        ...
        private void BtnApplyFilter_Click(object sender, RoutedEventArgs e)
        {
            ...
            LoadReport(from, to, outlet, empId, outId);
        ...
        private void BtnPrint_Click(object sender, RoutedEventArgs e)
        {
            ...
            doc.Blocks.Add(new Paragraph(new Run("Damage Report")) { FontSize = 16, FontWeight = FontWeights.Bold, Margin = new Thickness(0, 0, 0, 6) });
            ...
            string[] headers = { "SN", "REF NO", "DATE TIME", "ITEMS", "TOTAL LOSS", "RESPONSIBLE PERSON" };
            ...
            foreach (DataRowView rv in dv)
            {
                ...
                string[] vals = {
                    ...
                    rv["items"].ToString(),
                    "₹ " + Convert.ToDouble(rv["total_loss"]).ToString("N2"),
                    rv["responsible_person"].ToString()
                };
            ...
            dlg.PrintDocument(pag, "Damage Report");
        }
        ...
        private void BtnExportCsv_Click(object sender, RoutedEventArgs e)
        {
            ...
            var sfd = new Microsoft.Win32.SaveFileDialog { Filter = "CSV|*.csv", FileName = "DamageReport.csv" };
            if (sfd.ShowDialog() != true) return;

            using var sw = new StreamWriter(sfd.FileName, false, System.Text.Encoding.UTF8);
            sw.WriteLine("SN,REFERENCE NO,DATE TIME,ITEMS,TOTAL LOSS,RESPONSIBLE PERSON");
            foreach (DataRowView rv in dv)
            {
                sw.WriteLine(string.Join(",",
                    rv["sn"], rv["reference_no"], "\"" + rv["date_time"].ToString().Replace("\"", "\"\"") + "\"", rv["items"],
                    "\"" + Convert.ToDouble(rv["total_loss"]).ToString("N2") + "\"",
