...
{
    public partial class SupplierBalanceReportPage : UserControl
    {
        private readonly MainDashboard? _dashboard;
        private readonly DatabaseService _db = new();
        private DataTable? _currentTable;

        public SupplierBalanceReportPage(MainDashboard? dashboard)
        {
            InitializeComponent();
            ReportPdfHelper.Attach(exportPopup, () => dataGrid.ItemsSource as DataView, "Supplier Balance Report");
        ...
        private void LoadData()
        {
            ...
            try
            {
                ...
                // Get all suppliers
                var suppliers = new List<(long id, string name, string phone, double openingBalance, string openingType)>();
                using (var cmd = conn.CreateCommand())
                {
                    cmd.CommandText = "SELECT id, IFNULL(name,''), IFNULL(phone,''), IFNULL(opening_balance,0), IFNULL(opening_balance_type,'Debit')"
                        + " FROM suppliers WHERE (del_status IS NULL OR del_status='Live') ORDER BY name";
                    using var r = cmd.ExecuteReader();
                    while (r.Read())
                        suppliers.Add((r.GetInt64(0), r.GetString(1), r.GetString(2), r.GetDouble(3), r.GetString(4)));
                }
                ...
                foreach (var s in suppliers)
                {
                    ...
                    // Supplier: Cr opening = hum supplier ko dete hain (+), Dr opening = supplier humara deta hai (-).
                    bool isCreditOpening = s.openingType == "Cr" || s.openingType == "Credit";
                    double baseOb = isCreditOpening ? s.openingBalance : -s.openingBalance;

                    // Total purchases (Debit)
                    double totalPurchases = 0;
                    using (var cmd = conn.CreateCommand())
                    {
                        cmd.CommandText = "SELECT IFNULL(SUM(grand_total),0) FROM purchases WHERE supplier_id=@sid AND (del_status IS NULL OR del_status='Live')";
                        cmd.Parameters.AddWithValue("@sid", s.id);
                        var o = cmd.ExecuteScalar(); if (o != null) totalPurchases = Convert.ToDouble(o);
                    }
                    ...
                    using (var cmd = conn.CreateCommand())
                    {
                        cmd.CommandText = "SELECT IFNULL(SUM(amount),0) FROM supplier_payments WHERE supplier_id=@sid AND (del_status IS NULL OR del_status='Live')";
                        cmd.Parameters.AddWithValue("@sid", s.id);
                        var o = cmd.ExecuteScalar(); if (o != null) totalPayments = Convert.ToDouble(o);
                    }
                    ...
                    using (var cmd = conn.CreateCommand())
                    {
                        cmd.CommandText = "SELECT IFNULL(SUM(total_return_amount),0) FROM purchase_returns WHERE supplier_id=@sid AND (del_status IS NULL OR del_status='Live')";
                        cmd.Parameters.AddWithValue("@sid", s.id);
                        var o = cmd.ExecuteScalar(); if (o != null) totalReturns = Convert.ToDouble(o);
                    }

                    double currentBalance = baseOb + totalPurchases - totalPayments - totalReturns;

                    // Supplier convention: positive = Credit (we owe supplier), negative = Debit.
                    if (type == "Debit" && currentBalance >= 0) continue;
                    if (type == "Credit" && currentBalance <= 0) continue;

                    string supplierName = string.IsNullOrEmpty(s.phone) ? s.name : $"{s.name} (+91-{s.phone})";
                    string balLabel = currentBalance >= 0 ? "Credit" : "Debit";

                    table.Rows.Add(idx++.ToString(), supplierName, $"₹ {Math.Abs(currentBalance):N2} ({balLabel})");
                ...
                foreach (var row in rows)
                    table.Rows.Add(sn++.ToString(), row["supplier_name"], row["current_balance"]);
                ...
                lblRowCount.Text        = $"Showing {table.Rows.Count} suppliers";

                lblTotalBalance.Text   = $"₹ {Math.Abs(totalBalance):N2} ({(totalBalance >= 0 ? "Credit" : "Debit")})";
                totalsBar.Visibility   = Visibility.Visible;
            }
            catch (Exception ex)
            {
                emptyState.Visibility = Visibility.Visible;
                MessageBox.Show("Error: " + ex.Message, "Supplier Balance Report", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void ShowFilterInfo()
        {
            ...
            AddInfo("Report", "Supplier Balance Report");
        ...
        private void BtnExportCsv_Click(object sender, RoutedEventArgs e)
        {
            ...
            var dlg = new SaveFileDialog { Filter = "CSV (*.csv)|*.csv", FileName = "SupplierBalanceReport_" + DateTime.Today.ToString("yyyyMMdd") + ".csv" };
        ...
        private void BtnPrint_Click(object sender, RoutedEventArgs e)
        {
            ...
            doc.Blocks.Add(new Paragraph(new Run("Supplier Balance Report")) { FontSize = 16, FontWeight = FontWeights.Bold, Margin = new Thickness(0, 0, 0, 6) });
            ...
            dlg.PrintDocument(pag, "Supplier Balance Report");
        }
    }
}
