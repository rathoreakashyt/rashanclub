...
{
    public partial class ServiceSaleReportPage : UserControl
    {
        private readonly MainDashboard? _dashboard;
        private readonly DatabaseService _db = new();
        private DataTable? _currentTable;

        public ServiceSaleReportPage(MainDashboard? dashboard)
        {
            InitializeComponent();
            ReportPdfHelper.Attach(exportPopup, () => dataGrid.ItemsSource as DataView, "Service Sale Report");
        ...
        private void LoadItems()
        {
            ...
            try
            {
                using var conn = _db.GetConnection();
                using var cmd = conn.CreateCommand();
                cmd.CommandText = "SELECT Id, name, code FROM items WHERE type='Service_Product' AND (del_status IS NULL OR del_status='Live') ORDER BY name";
                using var r = cmd.ExecuteReader();
                while (r.Read())
                {
        ...
        private void LoadData()
        {
            ...
            var sql = "SELECT ROW_NUMBER() OVER (ORDER BY s.sale_date DESC, s.Id DESC) AS sn,"
                ...
                + " CASE WHEN i.parent_id IS NOT NULL THEN IFNULL((SELECT name FROM items WHERE id=i.parent_id), '') || ' ' ELSE '' END"
                + " || IFNULL(i.name, '') || CASE WHEN i.code IS NOT NULL AND i.code != '' THEN ' (' || i.code || ')' ELSE '' END AS item,"
                + " ROUND(IFNULL(sd.qty, 0), 2) AS quantity,"
                + " ROUND(IFNULL(sd.menu_price_with_discount, IFNULL(sd.menu_unit_price, 0)), 2) AS unit_price,"
                + " ROUND(IFNULL(sd.qty, 0) * IFNULL(sd.menu_price_with_discount, IFNULL(sd.menu_unit_price, 0)), 2) AS total"
                + " FROM sale_details sd"
                + " JOIN sales s ON s.Id = sd.sales_id"
                + " LEFT JOIN items i ON i.Id = sd.item_id"
                + " LEFT JOIN customers c ON c.Id = s.customer_id"
                + " WHERE i.type = 'Service_Product'"
                + " AND (sd.del_status IS NULL OR sd.del_status='Live')"
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show("Error: " + ex.Message, "Service Sale Report", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void ShowFilterInfo()
        {
            ...
            AddInfo("Report", "Service Sale Report");
        ...
        private void BtnExportCsv_Click(object sender, RoutedEventArgs e)
        {
            ...
            var dlg = new SaveFileDialog { Filter = "CSV (*.csv)|*.csv", FileName = "ServiceSaleReport_" + DateTime.Today.ToString("yyyyMMdd") + ".csv" };
        ...
        private void BtnPrint_Click(object sender, RoutedEventArgs e)
        {
            ...
            doc.Blocks.Add(new Paragraph(new Run("Service Sale Report")) { FontSize = 16, FontWeight = FontWeights.Bold, Margin = new Thickness(0, 0, 0, 6) });
            ...
            dlg.PrintDocument(pag, "Service Sale Report");
        }
    }
}
