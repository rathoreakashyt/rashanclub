...
{
    public partial class ServicingReportPage : UserControl
    {
        private readonly MainDashboard? _dashboard;
        private readonly DatabaseService _db = new();
        private DataTable? _currentTable;

        public ServicingReportPage(MainDashboard? dashboard)
        {
            InitializeComponent();
            ReportPdfHelper.Attach(exportPopup, () => dataGrid.ItemsSource as DataView, "Servicing Report");
        ...
        private void LoadData()
        {
            ...
            var sql = "SELECT ROW_NUMBER() OVER (ORDER BY s.date DESC, s.Id DESC) AS sn,"
                + " s.date,"
                + " IFNULL(s.delivery_date, '-') AS delivery_date,"
                + " IFNULL(c.name, '-') || CASE WHEN c.phone IS NOT NULL AND c.phone != '' THEN ' (' || c.phone || ')' ELSE '' END AS customer,"
                + " IFNULL(s.description, '-') AS details,"
                + " ROUND(IFNULL(s.servicing_charge, 0), 2) AS servicing_charge,"
            ...
            sql += " AND s.date BETWEEN @from AND @to";
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show("Error: " + ex.Message, "Servicing Report", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        {
            lblTotalCharge.Text = $"₹ {ColSum(t, "servicing_charge"):N2}";
        ...
        private void ShowFilterInfo()
        {
            ...
            AddInfo("Report", "Servicing Report");
        ...
        private void BtnExportCsv_Click(object sender, RoutedEventArgs e)
        {
            ...
            var dlg = new SaveFileDialog { Filter = "CSV (*.csv)|*.csv", FileName = "ServicingReport_" + DateTime.Today.ToString("yyyyMMdd") + ".csv" };
        ...
        private void BtnPrint_Click(object sender, RoutedEventArgs e)
        {
            ...
            doc.Blocks.Add(new Paragraph(new Run("Servicing Report")) { FontSize = 16, FontWeight = FontWeights.Bold, Margin = new Thickness(0, 0, 0, 6) });
            ...
            dlg.PrintDocument(pag, "Servicing Report");
        }
    }
}
