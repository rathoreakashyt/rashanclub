...
{
    public partial class StockReportPage : UserControl
    {
        private readonly MainDashboard? _dashboard;
        private readonly DatabaseService _db = new();
        private DataTable? _currentTable;

        public StockReportPage(MainDashboard? dashboard)
        {
            InitializeComponent();
            ReportPdfHelper.Attach(exportPopup, () => dataGrid.ItemsSource as DataView, "Stock Report");
        ...
        private void LoadData()
        {
            ...
            var sql = "SELECT ROW_NUMBER() OVER (ORDER BY i.name ASC) AS sn,"
                ...
                + " ROUND(IFNULL(i.stock_quantity, 0), 2) AS stock_qty_num,"
                + " CASE WHEN i.unit_type='2' AND IFNULL(i.conversion_rate,0) > 0 THEN IFNULL(i.purchase_price, 0) / i.conversion_rate ELSE IFNULL(i.purchase_price, 0) END AS lpp_num,"
                + " i.stock_quantity AS raw_stock,"
                + " CASE WHEN i.unit_type='2' AND IFNULL(i.conversion_rate,0) > 0 THEN IFNULL(i.purchase_price, 0) / i.conversion_rate ELSE IFNULL(i.purchase_price, 0) END AS calc_price"
                + " FROM items i"
            ...
            try
            {
                ...
                {
                    if (double.TryParse(row["raw_stock"]?.ToString(), NumberStyles.Any, CultureInfo.InvariantCulture, out double qty))
                        stockCount += qty;
                    if (double.TryParse(row["total"]?.ToString(), NumberStyles.Any, CultureInfo.InvariantCulture, out double val))
                        stockValue += val;
                }

                // Remove hidden calculation columns
                if (table.Columns.Contains("stock_qty_num")) table.Columns.Remove("stock_qty_num");
                if (table.Columns.Contains("lpp_num"))       table.Columns.Remove("lpp_num");
                if (table.Columns.Contains("raw_stock"))     table.Columns.Remove("raw_stock");
                if (table.Columns.Contains("calc_price"))    table.Columns.Remove("calc_price");

                _currentTable = table;

                lblStockValue.Text = $"INR{stockValue:N2}";
                lblStockCount.Text = stockCount.ToString("N2");
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show("Error: " + ex.Message, "Stock Report", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void BtnExportCsv_Click(object sender, RoutedEventArgs e)
        {
            ...
            var dlg = new SaveFileDialog { Filter = "CSV (*.csv)|*.csv", FileName = "StockReport_" + DateTime.Today.ToString("yyyyMMdd") + ".csv" };
        ...
        private void BtnPrint_Click(object sender, RoutedEventArgs e)
        {
            ...
            doc.Blocks.Add(new Paragraph(new Run("Stock Report")) { FontSize = 16, FontWeight = FontWeights.Bold, Margin = new Thickness(0, 0, 0, 6) });
            doc.Blocks.Add(new Paragraph(new Run($"Stock Value: {lblStockValue.Text}  |  Stock Count: {lblStockCount.Text}")) { Foreground = Brushes.Gray, Margin = new Thickness(0, 0, 0, 12) });
            ...
            dlg.PrintDocument(pag, "Stock Report");
        }
    }
}
