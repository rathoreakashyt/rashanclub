...
{
    public partial class ExpenseReportPage : UserControl
    {
        private readonly MainDashboard _dashboard;
        private readonly DatabaseService _db = new();
        private DataTable _allData = new DataTable();

        public ExpenseReportPage(MainDashboard dashboard)
        {
            InitializeComponent();
            ReportPdfHelper.Attach(exportPopup, () => dataGrid.ItemsSource as DataView, "Expense Report");
        ...
        private void LoadFilters()
        {
            ...
            try
            {
                ...
                cmd.CommandText = "SELECT id, name FROM expense_categories WHERE (del_status IS NULL OR del_status='Live') ORDER BY name";
        ...
        private void LoadReport(DateTime? dateFrom = null, DateTime? dateTo = null, long categoryId = 0, long employeeId = 0)
        {
            try
            {
                ...
                string dateCond(string col) => dateFrom.HasValue && dateTo.HasValue
                    ...
                    : "";
                string catCond = categoryId > 0 ? $" AND e.category_id={categoryId}" : "";
                string empCond = employeeId > 0 ? $" AND e.employee_id={employeeId}" : "";
                ...
                using (var cmd = conn.CreateCommand())
                {
                    cmd.CommandText = $@"SELECT e.id, e.reference_no, e.date, e.amount, e.category_id, e.employee_id
                        FROM expenses e
                    ...
                    while (r.Read())
                    {
                        ...
                        if (catId > 0)
                        {
                            ...
                            cc.CommandText = "SELECT name FROM expense_categories WHERE id=@cid AND (del_status IS NULL OR del_status='Live')";
        ...
        private void BtnPrint_Click(object sender, RoutedEventArgs e)
        {
            ...
            doc.Blocks.Add(new Paragraph(new Run("Expense Report")) { FontSize = 16, FontWeight = FontWeights.Bold, Margin = new Thickness(0, 0, 0, 6) });
            ...
            string[] headers = { "SN", "REF NO", "DATE TIME", "AMOUNT", "CATEGORY", "RESPONSIBLE PERSON" };
            ...
            dlg.PrintDocument(pag, "Expense Report");
        }
        ...
        private void BtnExportCsv_Click(object sender, RoutedEventArgs e)
        {
            ...
            var sfd = new Microsoft.Win32.SaveFileDialog { Filter = "CSV|*.csv", FileName = "ExpenseReport.csv" };
            if (sfd.ShowDialog() != true) return;

            using var sw = new StreamWriter(sfd.FileName, false, System.Text.Encoding.UTF8);
            sw.WriteLine("SN,REFERENCE NO,DATE TIME,AMOUNT,CATEGORY,RESPONSIBLE PERSON");
            foreach (DataRowView rv in dv)
            {
