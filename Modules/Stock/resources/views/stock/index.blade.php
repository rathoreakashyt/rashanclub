...
@section('page-title', __('Stock'))
@push('page-css')
<style>
    .table tr th:last-child {
        text-align: right !important;
    }
    .stock_modal_print_btn .dropdown-menu {
        ...
        <div class="card-body">
            <form id="filterForm" onsubmit="event.preventDefault(); return false;" action="javascript:void(0);">
                <div class="row">
                    ...
                    <div class="col-md-3 mb-3">
                        <label class="form-label">{{ __('Price') }} {{ __('Type') }}</label>
                        <select class="form-select select2" name="price_type" id="price_type_f">
                            <option value="last_purchase_price" {{ request('price_type') == 'last_purchase_price' ? 'selected' : '' }}>{{ __('Last Purchase Price') }}</option>
                            <option value="last_three_purchase_avg" {{ request('price_type', 'last_three_purchase_avg') == 'last_three_purchase_avg' ? 'selected' : '' }}>{{ __('Last 3 Purchase AVG') }}</option>
                        </select>
                    </div>
...
@push('page-js')
@routes
<script src="{{ asset('backend_assets/js/stock/stock.js') }}"></script>
@endpush
