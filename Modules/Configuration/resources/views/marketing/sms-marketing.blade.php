...
@section('page-title', __('SMS Marketing'))
@push('page-css')
...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-6 row-gap-4">
        <div class="d-flex flex-column justify-content-center">
            <h4 class="mb-0">{{ __('SMS Marketing') }}</h4>
        </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="flex-grow-1">
                            <h6 class="mb-0">{{ __('Customers with Phone') }}</h6>
                            <p class="mb-0 fs-4 fw-semibold" id="eligible_phone_count">0</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="row">
        ...
        <div class="col-12 col-lg-6 mb-4">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title mb-0">{{ __('Birthday SMS') }}</h5>
                </div>
                <div class="card-body">
                    <form id="birthday_sms_form">
                        @csrf
                        <div class="row">
                            <div class="col-12 mb-3">
                                <label class="form-label" for="birthday_message">{{ __('SMS Message') }} {!! requiredField() !!}</label>
                                ...
                                <small class="text-muted">
                                    {{ __('Character count:') }} <span id="birthday_char_count">0</span> / 160
                                </small>
                            </div>
                            <div class="col-12 mb-3">
                                <div class="alert alert-info">
                                    <p class="mb-2">{{ __('This SMS will be sent to customers celebrating their birthday today.') }}</p>
                                    <p class="mb-0">
                            ...
                            <div class="col-12">
                                <button type="submit" class="btn btn-primary" id="submit_birthday_sms">
                                    <i class="ti tabler-message-forward me-2"></i>
                                    {{ __('Send Birthday SMS') }}
                                </button>
                            </div>
                        </div>
                    </form>
        ...
        <div class="col-12 col-lg-6 mb-4">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title mb-0">{{ __('Anniversary SMS') }}</h5>
                </div>
                <div class="card-body">
                    <form id="anniversary_sms_form">
                        @csrf
                        <div class="row">
                            <div class="col-12 mb-3">
                                <label class="form-label" for="anniversary_message">{{ __('SMS Message') }} {!! requiredField() !!}</label>
                                <textarea class="form-control" 
                                ...
                                <small class="text-muted">
                                    {{ __('Character count:') }} <span id="anniversary_char_count">0</span> / 160
                                </small>
                            </div>
                            <div class="col-12 mb-3">
                                <div class="alert alert-info">
                                    <p class="mb-2">{{ __('This SMS will be sent to customers celebrating their anniversary today.') }}</p>
                                    <p class="mb-0">
                                        <strong>{{ __('Total Recipients:') }}</strong>
                            ...
                            <div class="col-12">
                                <button type="submit" class="btn btn-primary" id="submit_anniversary_sms">
                                    <i class="ti tabler-message-forward me-2"></i>
                                    {{ __('Send Anniversary SMS') }}
                                </button>
                            </div>
                        </div>
                    </form>
        ...
        <div class="col-12 col-lg-6 mb-4">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title mb-0">{{ __('Custom SMS') }}</h5>
                    <p class="text-muted mb-0">{{ __('Send a custom SMS to selected customers or everyone') }}</p>
                </div>
                <div class="card-body">
                    <form id="custom_sms_form">
                        @csrf
                        <div class="row">
                            <div class="col-12 mb-3">
                            ...
                            <div class="col-12 mb-3">
                                <label class="form-label" for="custom_message">{{ __('SMS Message') }} {!! requiredField() !!}</label>
                                <textarea class="form-control" 
                                    id="custom_message" 
                                    name="message"
                                    rows="6"
                                ...
                                <small class="text-muted">
                                    {{ __('Character count:') }} <span id="custom_char_count">0</span> / 160
                                </small>
                            </div>
                            <div class="col-12 mb-3">
                                <div class="alert alert-info">
                                    <p class="mb-2" id="custom_alert_message">
                                        {{ __('This SMS will be sent only to the customers you selected above.') }}
                                    </p>
                                    <p class="mb-0">
                            ...
                            <div class="col-12">
                                <button type="submit" class="btn btn-primary" id="submit_custom_sms">
                                    <i class="ti tabler-message-forward me-2"></i>
                                    {{ __('Send Custom SMS') }}
                                </button>
                            </div>
...
$(document).ready(function() {
    ...
    function fetchStats() {
        $.ajax({
            type: "GET",
            url: base_url + "/marketing/sms/stats",
            headers: {
    ...
    function updateStatsDisplay() {
        $('#birthday_recipients_count').text(stats.sms.birthday || 0);
        $('#anniversary_recipients_count').text(stats.sms.anniversary || 0);
        $('#eligible_phone_count').text(stats.sms.eligible || 0);
        $('#birthday_alert_count').text(stats.sms.birthday || 0);
        $('#anniversary_alert_count').text(stats.sms.anniversary || 0);
    }
    ...
    function fetchRecipients() {
        $.ajax({
            type: "GET",
            url: base_url + "/marketing/sms/recipients",
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            dataType: "json",
    ...
    function handleCustomRecipientsChange(selected) {
        ...
        if (selected.includes(ALL_RECIPIENTS_VALUE)) {
            ...
            $('#custom_alert_message').text('{{ __('This SMS will be sent to every customer with a valid phone number.') }}');
            $('#custom_alert_count').text(getCustomRecipientCount());
        } else {
            customSelection = selected;
            $('#custom_send_to_all').val('0');
            $('#custom_alert_message').text('{{ __('This SMS will be sent only to the customers you selected above.') }}');
            $('#custom_alert_count').text(getCustomRecipientCount());
        }
    }
    ...
    function loadRecommendedBirthday() {
        $.ajax({
            type: "GET",
            url: base_url + "/marketing/sms/recommended-message/birthday",
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            dataType: "json",
    ...
    function loadRecommendedAnniversary() {
        $.ajax({
            type: "GET",
            url: base_url + "/marketing/sms/recommended-message/anniversary",
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            dataType: "json",
    ...
    // Submit birthday SMS
    $('#birthday_sms_form').on('submit', function(e) {
        e.preventDefault();
        const btn = $('#submit_birthday_sms');
        const originalText = btn.html();
        btn.prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-2"></span>{{ __('Sending...') }}</span>');

        $.ajax({
            type: "POST",
            url: base_url + "/marketing/sms/send/birthday",
            data: $(this).serialize(),
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            ...
            success: function (response) {
                if (response.status === 'success') {
                    showSuccessNotification(response.message || '{{ __('Birthday SMS sent successfully') }}');
                    $('#birthday_sms_form')[0].reset();
                    updateCharCount('birthday_message', 'birthday_char_count');
                    fetchStats();
                } else {
                    showErrorNotification(response.message || '{{ __('Failed to send birthday SMS') }}');
                    if (response.errors) {
                        displayFormErrors('#birthday_sms_form', response.errors);
                    }
                }
            },
            error: function (xhr) {
                if (xhr.status === 422) {
                    displayFormErrors('#birthday_sms_form', xhr.responseJSON.errors);
    ...
    // Submit anniversary SMS
    $('#anniversary_sms_form').on('submit', function(e) {
        e.preventDefault();
        const btn = $('#submit_anniversary_sms');
        const originalText = btn.html();
        btn.prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-2"></span>{{ __('Sending...') }}</span>');

        $.ajax({
            type: "POST",
            url: base_url + "/marketing/sms/send/anniversary",
            data: $(this).serialize(),
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            ...
            success: function (response) {
                if (response.status === 'success') {
                    showSuccessNotification(response.message || '{{ __('Anniversary SMS sent successfully') }}');
                    $('#anniversary_sms_form')[0].reset();
                    updateCharCount('anniversary_message', 'anniversary_char_count');
                    fetchStats();
                } else {
                    showErrorNotification(response.message || '{{ __('Failed to send anniversary SMS') }}');
                    if (response.errors) {
                        displayFormErrors('#anniversary_sms_form', response.errors);
                    }
                }
            },
            error: function (xhr) {
                if (xhr.status === 422) {
                    displayFormErrors('#anniversary_sms_form', xhr.responseJSON.errors);
                    showErrorNotification('{{ __('Please check the form for errors') }}');
                } else {
                    showErrorNotification(xhr.responseJSON?.message || '{{ __('An unexpected error occurred') }}');
                }
    ...
    // Submit custom SMS (matching Vue handleSubmit logic)
    $('#custom_sms_form').on('submit', function(e) {
        e.preventDefault();
        const btn = $('#submit_custom_sms');
        const originalText = btn.html();
        btn.prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-2"></span>{{ __('Sending...') }}</span>');
        ...
        $.ajax({
            type: "POST",
            url: base_url + "/marketing/sms/send/custom",
            data: $.param(formData),
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            ...
            success: function (response) {
                if (response.status === 'success') {
                    showSuccessNotification(response.message || '{{ __('Custom SMS sent successfully') }}');
                    $('#custom_sms_form')[0].reset();
                    ...
                    showErrorNotification(response.message || '{{ __('Failed to send custom SMS') }}');
                    if (response.errors) {
                        displayFormErrors('#custom_sms_form', response.errors);
                    }
                }
            },
            error: function (xhr) {
                if (xhr.status === 422) {
                    displayFormErrors('#custom_sms_form', xhr.responseJSON.errors);
                    showErrorNotification('{{ __('Please check the form for errors') }}');
                } else {
                    showErrorNotification(xhr.responseJSON?.message || '{{ __('An unexpected error occurred') }}');
                }
