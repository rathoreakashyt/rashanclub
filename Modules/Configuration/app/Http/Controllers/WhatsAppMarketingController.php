...
class WhatsAppMarketingController extends Controller
{
    ...
    /**
     * Base query for customers in current company with valid phone (WhatsApp number).
     */
    ...
    /**
     * GET /marketing/whatsapp/stats — counts for birthday today, anniversary today, eligible (with phone).
     */
    ...
    /**
     * GET /marketing/whatsapp/recipients — list of customers with phone for multi-select (searchable).
     */
    ...
    /**
     * GET /marketing/whatsapp/recommended-message/{type} — default message for birthday or anniversary.
     */
    ...
    /**
     * POST /marketing/whatsapp/send/birthday — send WhatsApp to customers with birthday today.
     */
    public function sendBirthday(Request $request)
    {
        ...
        foreach ($recipients as $customer) {
            $result = $this->whatsappService->sendTestWhatsapp($customer->phone, $message, []);
            if (($result['status'] ?? '') === 'Success') {
                $sent++;
            } else {
                $failed++;
                Log::warning('Birthday WhatsApp failed', ['customer_id' => $customer->id, 'result' => $result]);
            }
        }

        return response()->json([
            'status' => 'success',
            'message' => __('Birthday WhatsApp sent: :sent. Failed: :failed.', ['sent' => $sent, 'failed' => $failed]),
            'data' => ['sent' => $sent, 'failed' => $failed],
        ]);
    }

    /**
     * POST /marketing/whatsapp/send/anniversary — send WhatsApp to customers with anniversary today.
     */
    public function sendAnniversary(Request $request)
    {
        ...
        foreach ($recipients as $customer) {
            ...
            if (($result['status'] ?? '') === 'Success') {
                ...
                $failed++;
                Log::warning('Anniversary WhatsApp failed', ['customer_id' => $customer->id, 'result' => $result]);
            }
        }

        return response()->json([
            'status' => 'success',
            'message' => __('Anniversary WhatsApp sent: :sent. Failed: :failed.', ['sent' => $sent, 'failed' => $failed]),
            'data' => ['sent' => $sent, 'failed' => $failed],
        ]);
    }

    /**
     * POST /marketing/whatsapp/send/custom — send WhatsApp to selected customers or all.
     */
    public function sendCustom(Request $request)
    {
        ...
        foreach ($recipients as $customer) {
            ...
            if (($result['status'] ?? '') === 'Success') {
                ...
                $failed++;
                Log::warning('Custom marketing WhatsApp failed', ['customer_id' => $customer->id, 'result' => $result]);
            }
        }

        return response()->json([
            'status' => 'success',
            'message' => __('Custom WhatsApp sent: :sent. Failed: :failed.', ['sent' => $sent, 'failed' => $failed]),
            'data' => ['sent' => $sent, 'failed' => $failed],
        ]);
    }
}
