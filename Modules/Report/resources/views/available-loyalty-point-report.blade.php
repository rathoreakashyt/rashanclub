...
@section('page-title', __('Available Loyalty Point Report'))
@push('page-css')
@endpush
...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-6 row-gap-4">
        <div class="d-flex flex-column justify-content-center">
            <h4 class="mb-0">{{ __('Available Loyalty Point Report') }}</h4>
        </div>
        @include('backend.components.breadcrumb', [
            'breadcrumbs' => [
                ['label' => '<i class="ti tabler-home"></i>', 'link' => route('report.index')],
                ['label' => __('Report'), 'link' => route('report.index')],
                ['label' => __('Available Loyalty Point Report'), 'active' => true]
            ]
        ])
    </div>
        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-datatable table-responsive pt-0">
                    <table class="datatables-basic table report-table" id="availableLoyaltyPointReportTable">
                        <thead>
                            <tr>
                                <th>{{ __('SN') }}</th>
                                <th>{{ __('Customer') }} ({{ __('Phone') }})</th>
                                <th>{{ __('Available Points') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                        </tbody>
...
@push('page-js')
@routes
<script src="{{ asset('backend_assets/js/report/available_loyalty_point_report.js') }}"></script>
@endpush
